...
# with open('../Source/headed_shots_output.pkl', 'rb') as file:
#    headers_output = pickle.load(file)

# with open('../Source/regular_shots_output.pkl', 'rb') as file:
#    regular_shots_output = pickle.load(file)

# player_values = pd.read_csv('player_value_matched')
# player_minutes = pd.read_csv("player_minutes.csv")
# player_data = player_values.merge(player_minutes[['player_name', 'minutes_played']], on='player_name')

# players_xg_reg = regular_shots_output.groupby(["player_name"])["shot_statsbomb_xg"].sum().sort_values(ascending=False).reset_index()
# players_xg_reg = players_xg_reg.rename(columns={'shot_statsbomb_xg': 'regular_shots_xg'})
# players_xg_head = headers_output.groupby(["player_name"])["shot_statsbomb_xg"].sum().sort_values(ascending=False).reset_index()
# players_xg_head = players_xg_head.rename(columns={'shot_statsbomb_xg': 'headed_xg'})
...
# pass_shot.loc[current_event.name, 'shot_statsbomb_xg'] = next_event["shot_statsbomb_xg"]
...
# pass_shot_wxg = pass_shot_wxg.groupby('player_name')['shot_statsbomb_xg'].sum().reset_index(name='total_xA')
# players_xg = merge_and_calculate(players_xg, pass_shot_wxg, 'total_xA', 'xA_p90')
# players_xg.fillna(0, inplace=True)
# players_xg.to_csv('player_xg.csv')

# Opening dataframe CSV
players_xg = pd.read_csv('player_xg.csv')
...
# Finding the top 10 players with the highest xG
top_10_players_g = players_xg.nlargest(8, 'xg_total')
...
#todo fix this scatter plot

# Scatter for xG vs actual goals
yticks = np.arange(0, 7, 1)
xticks = np.arange(0, 5, 1)
x_tick_labels = [str(int(tick / 1)) for tick in xticks]
cbar_ticks = np.arange(-1, 3, 1)
cbar_label = 'xG overperformance'
# Scatter for xg overperformance
player_scatter(xG, goals, xG_overperformance, top_10_players_op,
               yticks, xticks, x_tick_labels,
               cbar_ticks=cbar_ticks, cbar_label=cbar_label,
               title="Top 5 leagues: xG vs Actual Goals", xtitle='Expect Goals (xG)', ytitle='Actual Goals',
               atitle='difference', ayaxis='xg_total')

# Prepping scatter function use for xA scatter
yticks = np.arange(0, 7, 1)
xticks = np.arange(0, max(val) + 1000000, 1000000)
x_tick_labels = [str(int(tick / 1000000)) for tick in xticks]
cbar_ticks = np.arange(0, 6.5, 1)
cbar_label = 'xA'
...
df_col_tot = ['total_goals', 'xg_total', 'difference', 'total_shots', 'total_take_ons',
          'total_carries', 'total_crosses_attempted', 'total_assists', 'total_key_passes', 'total_xA']

df_col_p90 = ['xg_p90', 'xg_p90', 'difference', 'shots_p90', 'take_ons_p90',
          'carries_p90', 'crosses_attempted_p90', 'assists_p90', 'key_passes_p90', 'xA_p90']

att_names_tot = ["Non-Penalty Goals", "xG", "xG overperformance", "Shots", "Take-Ons",
             "Carries", "Crosses Attempted", "Assists", "Key Passes", "xA (Expected Assists)"]

att_names_p90 = ["Non-Penalty Goals P90", "xG P90", "xG overperformance", "Shots P90", "Take-Ons P90",
